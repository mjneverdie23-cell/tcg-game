extends Control
## Packs tab: the free daily pack (24 h cooldown) and the premium pack, with
## a staged five-card reveal. Coins are earned on the Shop tab and in battle;
## nothing here grants them.
...
func _open_pack(pack: Dictionary) -> void:
	...
	PlayerData.record_counter("packs_opened")
	PlayerData.record_counter("cards_gained", cards.size())
